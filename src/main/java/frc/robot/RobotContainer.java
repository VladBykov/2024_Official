...
public class RobotContainer {
  ...
  LimelightTags m_LimelightTags = new LimelightTags();
  ...
  private void configureBindings() {
    m_DriveTrain.setDefaultCommand(new SwerveDriveCommand(() -> driverController.getLeftY(), () -> driverController.getLeftX(), () -> driverController.getRightX(), m_DriveTrain));
    driverButtonX.whileTrue(new AlignCommand(m_DriveTrain, () -> frc.robot.subsystems.Limelight.angle));
    driverButtonRS.onTrue(m_DriveTrain.WheelzLock());
    driverButtonB.onTrue(m_DriveTrain.ZeroHeading()); // When the "B" button on the driver controller is clicked, it Reset's our gyro (will be renamed)
    driverButtonA.onTrue(m_DriveTrain.toggleFieldRelativeEnable()); // When the "A" button on the driver controller is clicked, it turns off field relative mode (might get rid of TODO)
    driverButtonOptions.onTrue(m_DriveTrain.resetPose2d()); // When the Options Button on the driver controller is clicked, it resets our robots position
    manipButtonLB.onTrue(m_DriveTrain.LowerArm()); // when LeftBumper pressed on the manipulating controller, it raises our armPosition (renamed variable soon)
    manipButtonRB.onTrue(m_DriveTrain.RaiseArm()); // when RightBumper pressed on the manipulating controller, it lowers our armPosition (renamed variable soon)
  }
  ...
  public Command getAutonomousCommand() {
    ...
    (12.1, 8).setKinematics(m_DriveTrain.m_kinematics); // we don't know our acceleration 
    ...
    trajectory, m_DriveTrain::GetPose2d, m_DriveTrain.m_kinematics, m_DriveTrain.m_xController, m_DriveTrain.m_yController, m_DriveTrain.m_thetaController, m_DriveTrain::setModuleStates, m_DriveTrain);
